...
function ResetPassword() {
  ...
  const handleSubmit = () => {
    ...
    setLoading(true);
    axios.post('http://127.0.0.1:8000/api/reset-password/', { token, password })
      .then(() => {
        setSuccess(true);
        setLoading(false);
      ...
      .catch(err => {
        setError(err.response?.data?.error || 'Something went wrong. Link may have expired.');
        setLoading(false);
      });
  };
      ...
      <div className="login-card">
        ...
        {success ? (
          ...
          <>
            ...
            {error && (
              <div style={{ background: 'rgba(233,69,96,0.15)', color: 'var(--accent)', padding: '10px 14px', borderRadius: '8px', fontSize: '13px', marginBottom: '16px' }}>
                {error}
              </div>
            )}
